...
* Для регрессионного тестирования запуска можно создать AVD с образом `system-images;android-28;google_apis;x86` и воспользоваться скриптом `android-app/tools/scripts/start_emulator.sh`. По умолчанию он стартует эмулятор в headless-режиме: `./android-app/tools/scripts/start_emulator.sh --avd codex-avd-api28`. Чтобы увидеть полноценное окно устройства под Xvfb, добавьте флаг `--with-gui` (при необходимости скрипт автоматически завернёт запуск в `xvfb-run`). При отсутствии `Xvfb` можно удалить флаг — эмулятор стартует с настоящим окном. На хостах без аппаратной виртуализации эмулятор завершится с сообщением о недоступности `/dev/kvm` — это ограничение окружения.
